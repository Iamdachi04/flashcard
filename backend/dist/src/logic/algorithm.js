...
function update(buckets, card, difficulty) {
    ...
    if (!targetSet) {
        ...
        const emptySetForNewBucket = new Set();
...
function computeProgress(buckets, history) {
    ...
    for (const record of history) {
        ...
        const isCorrect = difficulty === flashcards_1.AnswerDifficulty.Easy || difficulty === flashcards_1.AnswerDifficulty.Hard;
        if (isCorrect) {
            correctAnswers++;
        }
    }
